...
final class APIEndpointPhotosTests: XCTestCase {
    func testSearchPhotos() throws {
        ...
        XCTAssertEqual(request.path, "/v1/search?query=nature&orientation=landscape&size=large&color=red&locale=ko-KR&page=1&per_page=15")
    }
    ...
    func testCuratedPhotos() throws {
        ...
        XCTAssertEqual(request.method, .get)
        XCTAssertEqual(request.path, "/v1/curated?page=1&per_page=15")
    }
    ...
    func testGetPhoto() throws {
        ...
        XCTAssertEqual(request.method, .get)
        XCTAssertEqual(request.path, "/v1/photos/2014422")
    }
}
