...
final class APIEndpointVideosTests: XCTestCase {
    func testSearchVideos() throws {
        ...
        XCTAssertEqual(request.path, "/videos/search?query=nature&orientation=landscape&size=large&locale=ko-KR&page=1&per_page=15")
    }
    ...
    func testSearchVideosWithEmptyQuery() {
        XCTAssertThrowsError(try APIEndpoint.Videos.search(query: "")) { error in
            ...
            XCTAssertEqual(error, .missingRequiredParameter(name: "query"))
        }
    }
    ...
    func testPopularVideos() throws {
        ...
        XCTAssertEqual(request.method, .get)
        XCTAssertEqual(request.path, "/videos/popular?min_width=1024&min_height=1024&min_duration=1&max_duration=60&page=1&per_page=15")
    }
    ...
    func testGetVideo() throws {
        ...
        XCTAssertEqual(request.method, .get)
        XCTAssertEqual(request.path, "/videos/videos/2499611")
    }
}
